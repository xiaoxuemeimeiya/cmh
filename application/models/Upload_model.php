...
class Upload_model extends CI_Model
{
     ...
     * @param int $crop      是否裁剪0为不裁剪,1为裁剪
     */
    public function comment_upload($file_name = 'upfile', $width = '', $height = '', $crop = 0, $orientation = 0)
    {
        $width       = (int)$width;
        $height      = (int)$height;
        $crop        = (int)$crop;
        ...
        if ($this->upload->do_upload($file_name)) {
            $image_data = $this->upload->data();
            //gif图片不处理
            if (strtolower($image_data['file_ext']) != '.gif' && strtolower($image_data['file_ext']) != '.mp4') {
                //开始旋转图片start
                if (!empty($orientation)) {
                    $rotation_angle = 0;
                    switch($orientation) {
